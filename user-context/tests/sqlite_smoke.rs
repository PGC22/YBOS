#[cfg(feature = "sqlite")]
mod sqlite_tests {
    use ybos_user_context::{SqliteUserContextStore, UserContextStore, ContextEntry, ContextCategory, ContextQuery};
    use serde_json::json;
    use uuid::Uuid;

    #[tokio::test]
    async fn test_sqlite_store_roundtrip() {
        let store = SqliteUserContextStore::in_memory().unwrap();
